...
// Une fois les données collectées, celles-ci vont êtres encodées en hexadécimal et transmises via un module LoRa Wio-E5
...
const char* TCP_HOST = "192.168.1.1"; // Lors de l'utilisation de la véritable API, remplacer par : 150.100.100.20 
const int TCP_PORT = 8765; // Mettre 80 pour l'API
...
const unsigned long SEND_INTERVAL = 180000;  // Pour 3 minutes, modifiable 


// Permet de afficher des séparateurs dans la console pour une meilleure lisibilité
void printSeparator() {
  Serial.println("========================================");
}
...
void fetchWeatherDataAPI() {
  ...
  if (ethClient.connected()) {
    ...
    ethClient.println("GET /?command=DataQuery&uri=MeteoPau&format=json&mode=most-recent&p1=10 HTTP/1.1");
...
void setup() {
  ...
  Serial.println("\n[ETHERNET] Démarrage du DHCP...");

 if (!Ethernet.begin(mac)) {
    Serial.println("  DHCP a échoué - Utilisation d'une IP statique");
